...
if __name__ == '__main__':
    ...
    if not args.embedding_skip:
        ...
        for idx, task_name in enumerate(task_list):
            path = os.getcwd() + '/single_task/offline_data/{}/{}.pkl'.format(task_name, policy_quality[idx])
            # Load Dataloader
        ...
        if args.model == 'PGTE':
            ...
            model_path, reward_path, transition_path, behavior_path = trainer.save(os.getcwd() + '/results/models/PGTE', seed)
        elif args.model == 'TE':
            trainer = TETrainer(datasets, args)
            trainer.train(args.epochs)
            model_path = trainer.save(os.getcwd() + '/results/models/TE', seed)
        else:
            NotImplementedError()
            model_path = None
    else:
        if args.model == 'TE':
            model_path = os.path.join(os.getcwd() + '/results/models/TE', 'task_encoder_{}_seed_{}.jax'.format(args.mode, args.seed))
        elif args.model == 'PGTE':
            model_path =  os.path.join(os.getcwd() + '/results/models/PGTE', 'policy_task_encoder_{}_seed_{}.jax'.format(args.mode, args.seed))
            reward_path =  os.path.join(os.getcwd() + '/results/models/PGTE', 'reward_decoder_{}_seed_{}.jax'.format(args.mode, args.seed))
            transition_path =  os.path.join(os.getcwd() + '/results/models/PGTE', 'transition_decoder_{}_seed_{}.jax'.format(args.mode, args.seed))
            behavior_path =  os.path.join(os.getcwd() + '/results/models/PGTE', 'behavior_decoder_{}_seed_{}.jax'.format(args.mode, args.seed))
        else:
            model_path = None
