...
def td3_critic_update(key:Any, critic: Model, critic_target: Model, actor_target: Model,
    ...
    def critic_loss_generator(start_idx, end_idx):
        noise = jax.random.normal(key) * target_policy_noise
        noise = jnp.clip(noise, -target_noise_clip, target_noise_clip)
        next_actions = jnp.clip((actor_target({'obs': replay_data.next_observations['obs'][start_idx: end_idx],
                                              'task': replay_data.next_observations['task'][start_idx: end_idx]}) + noise), -1, 1)

        # Compute the next Q-values: min over all critics targets
        next_q_values = critic_target({'obs': replay_data.next_observations['obs'][start_idx: end_idx],
                                        'task': replay_data.next_observations['task'][start_idx: end_idx]}, next_actions)
        next_q_values = jnp.minimum(next_q_values[0], next_q_values[1])
        target_q_values = replay_data.rewards[start_idx: end_idx] + (1 - replay_data.dones[start_idx: end_idx]) * gamma * next_q_values


        def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
            # Get current Q-values estimates for each critic network
            current_q1, current_q2 = critic.apply_fn({'params': critic_params}, {'obs': replay_data.observations['obs'][start_idx: end_idx],
                                              'task': replay_data.observations['task'][start_idx: end_idx]}, replay_data.actions[start_idx: end_idx])
    ...
    critic_loss_list = []
    batch_size = replay_data.observations['obs'].shape[0]
    for i in range(10):
        critic_loss_fn = critic_loss_generator((batch_size // 10) * i,  (batch_size // 10) * (i + 1))
        critic_loss_list.append(critic_loss_fn)

    new_critic, info = critic.PCGrad(update_key, critic_loss_list)
    return new_critic, info


def td3_actor_update(key:Any, actor: Model, critic: Model, replay_data:ReplayBufferSamples, alpha: float, without_exploration: bool):
    key, update_key = jax.random.split(key, 2)
    def actor_loss_generator(start_idx, end_idx):
        obs = {'obs': replay_data.next_observations['obs'][start_idx: end_idx], 'task': replay_data.next_observations['task'][start_idx: end_idx]}
        if without_exploration:
            actions_pi = actor(obs)
            q1 = critic(obs, actions_pi)[0]
        ...
        def actor_loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
            # Compute actor loss
            actions_pi = actor.apply_fn({'params': actor_params}, obs)
            q_value = critic(obs, actions_pi)[0].mean()

            actor_loss = - q_value
            if without_exploration:
                bc_loss = jnp.mean(jnp.square(actions_pi - replay_data.actions[start_idx: end_idx]))
    ...
    batch_size = replay_data.observations['obs'].shape[0]
    for i in range(10):
        actor_loss_fn = actor_loss_generator((batch_size // 10) * i,  (batch_size // 10) * (i + 1))
        actor_loss_list.append(actor_loss_fn)

    new_actor, info = actor.PCGrad(update_key, actor_loss_list)
...
def _update_jit(rng: int, actor: Model, critic: Model, actor_target: Model, critic_target: Model, replay_data: ReplayBufferSamples,
    ...
    new_critic, critic_info = td3_critic_update(critic_key, critic, critic_target, actor_target, replay_data, gamma, target_policy_noise, target_noise_clip)

    if actor_update_cond:
        new_actor, actor_info = td3_actor_update(actor_key, actor, new_critic, replay_data, alpha, without_exploration)
...
class PCGrad(OffPolicyAlgorithm):
    """
    ...
    def _setup_model(self) -> None:
        super(PCGrad, self)._setup_model()
    ...
    def learn(
        ...
        return super(PCGrad, self).learn(
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(PCGrad, self)._excluded_save_params() + ["actor", "critic", "actor_target", "critic_target"]
