...
def td3_actor_update_bc(actor: Model, critic: Model, replay_data:ReplayBufferSamples, alpha: float, without_exploration: bool, bc_model:Model, seq:jnp.ndarray):
    actions = bc_model(replay_data.observations['obs'], replay_data.observations['task'], seq)
    ...
    def actor_loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        actor_loss = actor_loss + 0.001 * bc_loss

        return actor_loss, {'actor_loss': actor_loss, 'q_value': q_value, 'coef_lambda': 1}
...
@functools.partial(jax.jit, static_argnames=('gamma', 'tau', 'target_policy_noise', 'target_noise_clip', 'alpha',
                                             'without_exploration', 'actor_update_cond'))
def _update_jit_bc(rng: int, actor: Model, critic: Model, actor_target: Model, critic_target: Model, replay_data: ReplayBufferSamples,
                actor_update_cond: bool, tau: float, target_policy_noise: float, target_noise_clip: float, gamma: float, alpha: float, without_exploration: bool,
                bc_model:Model, seq:jnp.ndarray) -> Tuple[int, Model, Model, Model, Model, InfoDict]:

    rng, key = jax.random.split(rng, 2)
    new_critic, critic_info = td3_critic_update(key, critic, critic_target, actor_target, replay_data, gamma, target_policy_noise, target_noise_clip)

    if actor_update_cond:
        new_actor, actor_info = td3_actor_update_bc(actor, new_critic, replay_data, alpha, without_exploration, bc_model, seq)
        new_actor_target = target_update(new_actor, actor_target, tau)
        new_critic_target = target_update(new_critic, critic_target, tau)
    else:
        new_actor, actor_info = actor, {'actor_loss': 0, 'q_value': 0, 'coef_lambda': 1}
...
class TD3(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        #     if isinstance(self.replay_buffer, ModelBasedDictReplayBuffer) and not self.replay_buffer.full:
        #         print('data point:', self.replay_buffer.data_point, 'buffer_size:', self.replay_buffer.buffer_size)
        #         self.replay_buffer.add_augmentation((self.replay_buffer.buffer_size - self.replay_buffer.pos), env=self._vec_normalize_env,
        #                                             verbose=1)
        ...
        for _ in range(gradient_steps):
            ...
            if self.bc_model is None:
                ...
                seq = np.zeros((batch_size, 8))
                seq[:, 4] = 1
                self.key, new_actor, new_critic, new_actor_target, new_critic_target, info = \
                _update_jit_bc(key, self.actor, self.critic, self.actor_target, self.critic_target, replay_data,
                            actor_update_cond, self.tau, self.target_policy_noise, self.target_noise_clip, self.gamma, self.alpha, self.without_exploration,
                            self.bc_model, seq)
            ...
            critic_losses.append(copy.deepcopy(info['critic_loss']))
            coef_lambda.append(info['coef_lambda'])

        # if self.data_augmentation:
        #     if isinstance(self.replay_buffer, ModelBasedDictReplayBuffer):
        #         self.replay_buffer.add_augmentation(gradient_steps//4, env=self._vec_normalize_env)
