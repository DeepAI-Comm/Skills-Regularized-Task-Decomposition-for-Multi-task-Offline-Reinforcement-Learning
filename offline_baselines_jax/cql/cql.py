...
def sac_critic_update(key:Any, actor: Model, critic: Model, critic_target: Model, log_ent_coef: Model,
    ...
    cql_log_prob = jnp.expand_dims(jnp.repeat(cql_log_prob, repeats=5, axis=0), axis=1)
    # repeated_observations = jnp.repeat(replay_data.observations, repeats=10, axis=0)
    repeated_observations = dict()
    repeated_observations['obs'] = jnp.repeat(replay_data.observations['obs'], repeats=5, axis=0)
    repeated_observations['task'] = jnp.repeat(replay_data.observations['task'], repeats=5, axis=0)
    key, subkey = jax.random.split(key, 2)
    random_actions = jax.random.uniform(subkey, minval=-1, maxval=1, shape=(batch_size * 5, action_dim))
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        cql_q1 = jnp.repeat(cql_q1, repeats=5, axis=0)
        cql_q2 = jnp.repeat(cql_q2, repeats=5, axis=0)
        random_q1, random_q2 = critic.apply_fn({'params': critic_params}, repeated_observations, random_actions)

        conservative_loss_1 = jax.scipy.special.logsumexp(jnp.concatenate([cql_q1 - cql_log_prob, random_q1 - random_density])).mean() - current_q1.mean()
        conservative_loss_2 = jax.scipy.special.logsumexp(jnp.concatenate([cql_q2 - cql_log_prob, random_q2 - random_density])).mean() - current_q2.mean()
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        if self.data_augmentation:
            if isinstance(self.replay_buffer, ModelBasedDictReplayBuffer) and not self.replay_buffer.full:
                ...
                self.replay_buffer.add_augmentation((self.replay_buffer.buffer_size - self.replay_buffer.pos),
