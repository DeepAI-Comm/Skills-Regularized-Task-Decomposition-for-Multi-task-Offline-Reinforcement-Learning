...
@functools.partial(jax.jit, static_argnames=('actor_apply_fn'))
def sample_actions_td3(actor_apply_fn: Callable[..., Any], actor_params: Params,
                    observations: np.ndarray, module_select:List[List[int]]) -> jnp.ndarray:
...
class SoftModuleTD3Policy(TD3Policy):
    """
    Policy class (with both actor and critic) for SAC.
    ...
    def __init__(
        ...
        super(SoftModuleTD3Policy, self).__init__(
            key,
            observation_space,
            action_space,
            lr_schedule,
    ...
    def _predict(self, observation: jnp.ndarray, module_select: List[List[int]]=None,) -> np.ndarray:
        actions = sample_actions_td3(self.actor.apply_fn, self.actor.params, observation, module_select)
        actions = np.asarray(actions)
        return actions
