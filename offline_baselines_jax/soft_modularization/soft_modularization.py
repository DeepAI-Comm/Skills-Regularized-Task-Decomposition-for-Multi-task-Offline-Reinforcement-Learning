...
def sac_critic_update(key:Any, actor: Model, critic: Model, critic_target: Model, log_ent_coef: Model,
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        critic_loss = 0.5 * (jnp.mean(loss_balancing_weight * jnp.square(current_q1 - target_q_values))
                             + jnp.mean(loss_balancing_weight * jnp.square(current_q2 - target_q_values))) + alpha_coef * conservative_loss
        ################################
...
class SoftModularization(OffPolicyAlgorithm):
    ...
    def learn(
        ...
        tb_log_name: str = "SoftModularization",
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(SoftModularization, self)._excluded_save_params() + ["actor", "critic", "critic_target", "log_ent_coef", "log_alpha_coef"]
