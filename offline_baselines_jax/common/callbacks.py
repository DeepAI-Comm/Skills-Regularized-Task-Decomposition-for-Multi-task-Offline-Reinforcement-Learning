...
class EvalCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            make_csv(task_rewards, task_lengths, self.best_model_save_path, self.num_timesteps)
...
def make_csv(task_rewards, task_success, path, num_timesteps):
    import csv

    task_name = ['tasks'] + list(task_rewards.keys())
    task_reward_list = ['reward']
    task_success_rate_list = ['success rate']
    for tn in task_rewards.keys():
        task_reward_list.append(float(np.mean(task_rewards[tn])))
        task_success_rate_list.append(float(np.mean(task_success[tn])))

    with open(os.path.join(path, 'result_log_{}.csv'.format(num_timesteps)), 'w', newline='') as f:
        wr = csv.writer(f)
        wr.writerow(task_name)
        wr.writerow(task_reward_list)
        wr.writerow(task_success_rate_list)
